...
매일 아침 카카오톡 '나와의 채팅'으로 명작을 **챕터 한 도막(약 2,000~2,500자)**
분량씩 이어서 보내주는 자동 발송기. 매일 같은 시각에 다음 회차가 오므로,
시간을 들여 한 권을 끝까지 읽어나가는 연재 방식으로 동작한다.
...
seq       = (그날까지 해당 트랙이 등장한 횟수) % 트랙 회차 수
```

`seq`로 고른 **회차 하나**(~2,000~2,500자)가 그날 발송 전체다. 카카오
...
- 트랙 안 여러 작품은 등록 순서대로 이어진다 (예: 봄·봄 완결 → 운수 좋은 날 시작)
